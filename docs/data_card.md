# Data Card — African Language Wikipedia Corpus for SuperBPE Tokenization

**Project:** Bridging the Tokenization Gap in African LLMs via SuperBPE
**Program:** TRI AI Saturdays
**Status:** 8 of 8 target languages complete (Swahili, Chichewa, Zulu,
Kinyarwanda, Hausa, Amharic, Yoruba, Wolof — per Section 7 of the project
plan), plus Igbo as a documented bonus 9th language beyond the required
set. Wolof required a second, supplementary source beyond Wikipedia to
reach an adequate corpus size; this is documented in Sections 2.1 and 6,
consistent with the collection plan's risk mitigation for uneven corpus
...
Standard BPE tokenizers, trained mostly on English-dominated web text,
over-fragment morphologically rich and agglutinative African languages,
inflating token counts and degrading downstream LLM performance. This
corpus was collected to train and evaluate SuperBPE (Liu et al., 2025,
arXiv:2503.13423) tokenizers on real African-language text, to measure
whether "superword" tokenization meaningfully reduces fragmentation and
improves efficiency across language families with different morphological
structures (Bantu agglutination, Volta-Niger and West Atlantic tonal
marking, Chadic concatenative morphology, Semitic root-and-pattern
morphology with a non-Latin script).
...
| Igbo (ig) *(bonus)* | Volta-Niger | Latin | Wikipedia | 57,457 | 53,686 | 93.4% |
...
| Wolof (wo) † | West Atlantic | Latin | Wikipedia + MasakhaNER | 2,351 | 3,721 | see 2.1 |
| **Total** | | | | **425,268** | **286,135** | mixed sources — see per-row |
...
substantially by language — this reflects genuine differences in how each
language's Wikipedia is composed (see Section 6), not inconsistent
cleaning.

† Wolof's "Raw extracted" and "Retention" columns reflect the Wikipedia
portion only. "Final cleaned" (3,721) is the combined total after adding a
supplementary source — see breakdown below.
...
spot-check as likely insufficient before cleaning even confirmed it,
consistent with the collection plan's instruction to flag insufficient
data before drawing conclusions. It was supplemented with MasakhaNER 1.0's
Wolof split (Adelani et al., 2021; `github.com/masakhane-io/masakhane-ner`),
a human-annotated NER dataset sourced from local news articles, licensed
`CC-BY-4.0-NC`. NER tags were discarded and sentences reconstructed as
plain text; train/dev/test splits were combined since the original task
split is irrelevant once the data is used as raw tokenizer training text
rather than labeled NER examples.
...
Kinyarwanda's (1,931,619) and every other language except Chichewa —
...
1. Register the source (license, access method, expected coverage) in the
   project's Source Registry before any data was pulled.
...
Rules 1–8 below were applied identically across all Wikipedia-sourced
languages, including Kinyarwanda and the Wikipedia portion of Wolof:

1. **Minimum word count filter** (<15 words dropped) — addresses empty
   ...
   Igbo/Hausa's sample vs. 50% of Amharic's vs. 60% of Wolof's — see
   Section 6).
2. **Encoding sanity check** — drops documents matching known mojibake
   patterns. Zero triggered for Kinyarwanda; zero for Wolof.
3. **Exact deduplication** — content-hash based, catches verbatim repeats.
4. **Near-duplicate template capping** — masks numbers/proper nouns into a
   ...
   distinct templates — including a newly observed pattern of
   ...
   underlying risk).
5. **Outlier-length truncation** (99th percentile cap) — mirrors the
   mitigation used in the original SuperBPE paper after they found a
   single document duplicated 2,224 times distorted their tokenizer
   training.
6. **Leaked XML revision-metadata removal** — found in Zulu's spot-check:
   ...
   Wolof, but **505 instances in Amharic** — a genuinely large gap worth
   flagging, though its underlying cause is not established (see
   Section 6). Fix verified correct at every observed frequency.
7. **Leaked templatestyles/infobox markup removal** — found in Igbo's
   ...
   1 in Wolof.
8. **Citation marker removal** — strips leftover `[1]`, `[2][3]` footnote
...
rules 1–8): a minimum sentence length of 3 words (vs. 15 for Wikipedia
...
can verify a given corpus file was never touched during training, per the
project's requirement that eval data be held out before anything else
touches the data.
...
- **Empty-page extraction rates vary hugely by language and are a real
  ...
  were disproportionately bare year-number articles (e.g. "229 እ.ኤ.አ.",
  "1938") — infobox-only date pages with zero prose. Wolof's failures were
...
- **Wolof required a supplementary source and remains the smallest corpus
  ...
  Chichewa's 204,640 and every other language's 1M+). Documented per the
  collection plan's instruction to report shortfalls rather than pad or
  conceal them.
- **Chichewa Wikipedia is extremely small**: only 1,408 raw articles,
  204,640 words after cleaning — well short of this project's 500,000-token
  working target. Documented honestly per the collection plan's instruction
  to report shortfalls rather than pad them.
- **Tone/diacritic marking is inconsistently applied within the Yoruba and
  ...
  (e.g. Igbo "amụrụ" vs. "amuru" for "born"). Cannot be corrected
  automatically without a real morphological analyzer or native-speaker
  review, per the project's morphological rule toolkit consultation
  protocol (Section B.3). Hausa, Amharic, Kinyarwanda, and Wolof showed no
  equivalent pattern in their own orthographic systems in spot-check
  samples.
- **Leaked XML revision-metadata frequency is unexplained and highly
  uneven**: 505 instances in Amharic vs. 0–11 in every other language. The
...
- **Non-Latin script (Amharic/Ethiopic) introduced no encoding defects.**
  ...
  sample — the pipeline's encoding handling generalized correctly to a
  completely different writing system with no code changes required. The
  same holds for Kinyarwanda and Wolof's Latin-script diacritics (`ë`,
  `à`, `ñ`, `ŋ`, apostrophe-marked forms), which rendered cleanly with
...
- **Retention rate is not correlated with corpus size.** Hausa (114,318
  ...
  smallest Wikipedia source in this corpus) retained under 50% —
  article count alone does not predict data quality or usability.

## 8. Maintenance

This corpus was assembled by the Data Engineering role for this project.
Sources, cleaning code, and spot-check reports are version-controlled
alongside this Data Card (`TRI-Project/` on the team's shared Drive). All
